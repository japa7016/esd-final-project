...
void ssd1306_TestFonts3() {
    // Step 1: Draw the full grid (horizontal and vertical lines)
    for (int x = 0; x <= 128; x += 16) { // Vertical lines
        ssd1306_Line(x, 0, x, 64, White);
    }
    for (int y = 0; y <= 64; y += 16) { // Horizontal lines
        ssd1306_Line(0, y, 128, y, White);
    }

    // Step 2: Overlay text on top of the grid
    ssd1306_SetCursor(2, 0); // Place text at the top-left
    ssd1306_WriteString("0V/div", Font_7x10, White);

    ssd1306_SetCursor(80, 0); // Top-right corner for Trigger info
    ssd1306_WriteString("Trig: -", Font_7x10, White);

    ssd1306_SetCursor(80, 10); // Just below "Trig: -" for Mode info
    ssd1306_WriteString("Mode: None", Font_7x10, White);
